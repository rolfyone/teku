  ...
  @Test
    void consolidatedValidatorsMoreLikelyToPropose() {
      ...
      final BeaconState preState = dataStructureUtil.randomBeaconState(8);
        final BeaconStateAccessorsElectra beaconStateAccessors = new BeaconStateAccessorsElectra(spec.getGenesisSpecConfig(), new PredicatesElectra(spec.getGenesisSpecConfig()), miscHelpersElectra);
      final BeaconState state = BeaconStateElectra.required(preState)
              .updated(
              mutableState -> {
                  mutableState.getValidators().set(3,
                          dataStructureUtil
                                  .validatorBuilder()
                  ...
                  mutableState.getBalances().set(3, SszUInt64.of(UInt64.THIRTY_TWO_ETH.times(consolidationAmount)));
                  mutableState.setSlot(UInt64.valueOf(8));

              });
      final Map<UInt64, Integer> proposerMap = new HashMap<>();
      final IntList indices = IntArrayList.of(1,2,3,4,5,6,7,0);
    for (int i = 1; i <  8; i++) {
        final UInt64 slot = UInt64.valueOf(8+i);
        final Bytes32 seed =
                Hash.sha256(beaconStateAccessors.getSeed(state, UInt64.ONE, Domain.BEACON_PROPOSER), uint64ToBytes(slot));

        proposerMap.put(slot, miscHelpersElectra.computeProposerIndex(state, indices, seed));
      }
    assertTrue(true);
  }
}
